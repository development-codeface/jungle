                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="active">
                                <!-- <i class="fa fa-fw fa-table"></i> --> Bookings
                            </li>
                        </ol>
                        <!-- BreadCrumbs << -->
                    </div>
...
<div class="col-lg-12">
								<div class="row">
									...
									<div class="col-lg-2">
										<div class="form-group">
											<select class="col-lg-8 form-control" name="status" id="status">
												<option value="0" <?php if(!empty($status)) { if($status == 0) echo 'selected'; } ?>>Blocked</option>
												<option value="1" <?php if(!empty($status)) { if($status == 1) echo 'selected'; } ?>>Reserved</option>
											</select>
										</div>
									</div>
									<div class="col-lg-3">
                            		...
                            		<a href="<?php echo base_url() . 'report/bookingsExcel?b_no='.$b_no.'&name='.$name.'&source='.$source.'&agency='.$agency.'&from='.$from.'&to='.$to.'&status='.$status; ?>"><img src="<?php echo base_url() . 'assets/img/excel.png'; ?>"/></a>
									<?php } ?>
									</div>
...
foreach ($results as $a_result) { 
	...
	else if($a_result -> agency != 0) { $source = 'Agency'; }
	
	if($a_result -> reservation != 0) { $source = 'Hotel'; }
	
	if($a_result -> status == 0) { $status = 'Blocked'; } else { $status = 'Reserved'; }
