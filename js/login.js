...
$('#coup_apply').click(function() {
	var code = $("#coup_code").val();
	var c_in =  getQueryVariable("check_in");
	var c_out =  getQueryVariable("check_out");
	if(code) {
	$('#cp_msg').text('');
	$.ajax({
		type : "post",
		url : baseurl + "checkout/coupon/" + code + "/" + c_in + "/" + c_out,
...
$('#cp_msg').text('This coupon is not valid');
}
		}
	});
	}
