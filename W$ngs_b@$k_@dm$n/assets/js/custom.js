...
$('#access_type').change(function(){
	...
	$.ajax({
		url: base_url+"Hotel_access/access_type/"+this.value,
		success: function(data) {
			$('#h_keyword').html(data);
		}
		});
...
$('#h_keyword').change(function(){
	$.ajax({
		url: base_url+"Hotel_access/access_options/"+this.value,
		success: function(data) {
			$('.hotel_fac').html(data);
...
if(id) {
	$.ajax({
		url: base_url + "Hotel_access/user_permission",
		method: "post",
		data: { id : id },
		success: function(data) {
		$("#menu").html(data);
