...
class Packages extends CI_Controller {
	...
	$user_id = $this->session->userdata['user_id'];
		...
		$this -> load -> view('layout/header');
		if($this->session->userdata['user_type']=='staff') {
		if(!empty($data_menu['menu']) && (in_array('9', json_decode($data_menu['menu']))==1)) {
	...
	$user_id = $this->session->userdata['user_id'];
		...
		$data_menu['menu'] = $this->Settingsmodel->select_user_menu($user_id); 
				...
				else
				{
					...
					if(!empty($data_menu['menu']) && (in_array('10', json_decode($data_menu['menu']))==1)) {
					$this -> load -> view('packages/add',$data);
					} else {
					$this->load->view('settings/no_permission');
					}
	...
	$user_id = $this->session->userdata['user_id'];
		...
		else
		{
			...
			if(!empty($data_menu['menu']) && (in_array('11', json_decode($data_menu['menu']))==1)) {
...
if(!empty($data_menu['menu']) && (in_array('12', json_decode($data_menu['menu']))==1)) {
...
if($this->session->userdata['user_type']=='staff') {
if(!empty($data_menu['menu']) && (in_array('13', json_decode($data_menu['menu']))==1)) {
