...
class Hotel extends CI_Controller {
	...
	function view_access($cat, $hotel) {
	...
	$user_id = $this->session->userdata['user_id'];
		...
		$data['groups'] = $this->Hotelmodel->allgroupdetails();
		...
		{			
			 $data['key'] = $this -> input -> get('g_keyword');
			 
		$config['suffix']		= '?' . http_build_query($_GET, '', "&"); 
		$config['base_url'] 	=  site_url('hotel/category/'.$name);
		$config['first_url']	= $config['base_url'].'?'.http_build_query($_GET);
		$config['total_rows'] 	=   $this -> Hotelmodel -> get_group_hotels_count($cat_id->id,$data['key']); 
		$config['per_page'] 	= 20;
		...
		$data['results'] = $this->Hotelmodel->get_group_hotels($cat_id->id,$data['key'],$config["per_page"], $data['page']);
		}
		...
		if($this->session->userdata['user_type']=='staff') {
		if(!empty($data_menu['menu']) && (in_array('1', json_decode($data_menu['menu']))==1)) {
		$this->load->view('hotel/list',$data);
		} else {
	...
	{	$user_id = $this->session->userdata['user_id'];
		...
		if($this->session->userdata['user_type']=='staff') {
		if(!empty($data_menu['menu']) && (in_array('3', json_decode($data_menu['menu']))==1)) {
	...
	$user_id = $this->session->userdata['user_id'];
		...
		if(!empty($data_menu['menu']) && (in_array('2', json_decode($data_menu['menu']))==1)) {
	...
	public function changestatus($id,$category)
	{
		...
		if(!empty($data_menu['menu']) && (in_array('6', json_decode($data_menu['menu']))==1)) {
		$datas['status']=0;
...
// 					
	...
	$user_id = $this->session->userdata['user_id'];
		...
		if(!empty($data_menu['menu']) && (in_array('4', json_decode($data_menu['menu']))==1)) {
