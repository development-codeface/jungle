...
class Report extends CI_Controller
{
	...
	function hotel_log()
	{
		...
		$config['total_rows'] 	=  $this->Reportmodel->hotel_log_count($data['from_date'],$data['to_date'],$dat);
		...
		$data['report'] = $this -> Reportmodel -> hotel_log($data['from_date'],$data['to_date'],$dat,$config["per_page"], $data['page']);
		}
...
if($this->session->userdata['user_type']=='staff') {
if(!empty($data_menu['menu']) && (in_array('22', json_decode($data_menu['menu']))==1)) {
	...
	$user_id = $this->session->userdata['user_id'];
		...
		$data_menu['menu'] = $this->Settingsmodel->select_user_menu($user_id); 
			...
			if(!empty($data_menu['menu']) && (in_array('21', json_decode($data_menu['menu']))==1)) {
...
if($this->session->userdata['user_type']=='staff') {
if(!empty($data_menu['menu']) && (in_array('23', json_decode($data_menu['menu']))==1)) {
	...
	$user_id = $this->session->userdata['user_id'];
		...
		$this -> load -> view('layout/header');
		if($this->session->userdata['user_type']=='staff') {
if(!empty($data_menu['menu']) && (in_array('42', json_decode($data_menu['menu']))==1)) {
		$this->load->view('report/side_menu');
...
if($this->session->userdata['user_type']=='staff') {
if(!empty($data_menu['menu']) && (in_array('25', json_decode($data_menu['menu']))==1)) {
		$this->load->view('report/side_menu');
		$this -> load -> view('report/amend_report',$data);
		} else {
		$this->load->view('settings/no_permission');
...
if($this->session->userdata['user_type']=='staff') {
if(!empty($data_menu['menu']) && (in_array('24', json_decode($data_menu['menu']))==1)) {
			$this->load->view('report/side_menu');
			$this -> load -> view('report/reservation_report',$data);
			} else {
			$this->load->view('settings/no_permission');
