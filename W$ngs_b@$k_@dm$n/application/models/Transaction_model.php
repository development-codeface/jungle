...
## bookingwings coupon ##
	...
	function couponDate($from, $to, $type) {
		return $this -> db
		-> where('(("'.$from. '" BETWEEN valid_from and valid_to) or ("'.$to. '" BETWEEN valid_from and valid_to)) and type = "' .$type.'"')
		-> get('tbl_site_coupon')
		-> num_rows();
	}
	...
	function allCoupons($limit,$start) {
		return $this -> db
		-> where('status', 1)
		-> order_by('id', 'desc')
		-> limit($limit,$start)
...
## bookingwings coupon end ##
   ...
   function getnumber($id, $c_in, $c_out, $limit, $start)
   {
	...
	$this->db->distinct();
   	$this->db->select('booking_number, offline_user, category');
   ...
   {
	  // echo $booking_number;
	    /* echo $sql = "select a.id as booking_id,a.date,a.booking_number,a.check_in,a.check_out,a.number_of_rooms,
		a.discount,a.payment,a.tax,a.total_amount,a.child_rate,a.bed_rate,a.status,a.number_of_childrens,a.hotel_id,		b.price,b.room_child_rate,b.allowed_persons,c.first_name,c.phone,c.address,c.email,d.room_title,a.commision,a.room_rate from tbl_booking as a join tbl_hotel_room_settings as b on b.id=a.room_id join tbl_user as c on c.id = a.user_id join tbl_hotel_room as d on d.id=b.room_id where a.booking_number = '".$booking_number."' and  a.hotel_id='".$id."' and  a.status = '0' or a.status = '1'";
	   ...
	   if(empty($id)){
		...
		b.price,b.room_child_rate,b.allowed_persons,c.first_name,c.phone,c.address,c.email,d.room_title,e.title as hotel,a.commision,a.room_rate');
		...
		$this->db->join('tbl_hotel as e','e.id=a.hotel_id');
		...
		return $query1->result();  
		 ...
		 $this->db->select('a.id as booking_id,a.date,a.booking_number,a.check_in,a.check_out,a.number_of_rooms,
		a.discount,a.payment,a.tax,a.total_amount,a.child_rate,a.bed_rate,a.status,a.number_of_childrens,a.hotel_id,
		b.price,b.room_child_rate,b.allowed_persons,c.first_name,c.phone,c.address,c.email,d.room_title,e.title as hotel,a.commision,a.room_rate');
		$this->db->from('tbl_booking as a');
		$this->db->join('tbl_hotel_room_settings as b','b.id=a.room_id');
		$this->db->join('tbl_user as c','c.id=a.user_id');
