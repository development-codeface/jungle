...
class Reservation_checkout_model extends CI_Model
{
			...
			$flag=0;
			 ...
			 { 
				...
				c.normal_allowed_kids='.$hotel_in_arr_new[$x]->normal_allowed_kids.' ';
				 ...
				 $sql_new_qry_out= '(select c.* from tbl_hotel_room_settings as c  where c.room_id='.$r_id.' and c.available_status=1 '.$date_checkout.'  '.$select_conditions.'  )';
	...
	function Package_settingsByRoom($id)
	{
		...
		$this->db->where('room_id', $id);$this->db->where('available_status', 1);
			...
			$flag=0;
			 ...
			 { 
				...
				c.normal_allowed_kids='.$hotel_in_arr_new[$x]->normal_allowed_kids.' ';
				 ...
				 $sql_new_qry_out= '(select c.* from tbl_ayurveda_rooms as c  where c.room_id='.$r_id.' and c.available_status=1 and c.ayurveda_id='.$ayur_id.'   '.$date_checkout.'  '.$select_conditions.'  )';
				$query_out_exe_rooms = $this->db->query($sql_new_qry_out);
				$hotel_out_arr_new=$query_out_exe_rooms->result(); 
