...
// 		
	...
	function all_hotels_search_based_destination($post, $star='',$price_range='', $loc_range='' , $fac_range='', $hotName_range='')
	{
		...
		on e.hotel_id=a.id where ((a.destination like "%'.$keyword.'%" )) '.$dat_query.' '.$text.'and c.available_status=1 group by a.title)';
		...
		$sql_qry1='(select '.$select_values.' from tbl_hotel as a left join tbl_hotel_room as b on a.id=b.hotel_id left join
		tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
		on e.hotel_id=a.id where ((a.destination like "%'.$keyword.'%")) '.$dat_query.' '.$text_room.' and c.available_status=1 group by b.id)';
		...
		tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
		on e.hotel_id=a.id where '.$selescted_id_date.' ((a.destination like "%'.$keyword.'%" )) '.$date_checkin.' '.$text.'and c.available_status=1 )'; 
		...
		tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
		on e.hotel_id=a.id where '.$selescted_id_date.' ((a.destination like "%'.$keyword.'%" )) '.$date_checkout.' '.$text.' '.$select_conditions.'and c.available_status=1 )';
			...
			if(!empty($get_id))
			{
			   $sql_new_qry_date= '(select '.$select_values.' from tbl_hotel as a left join tbl_hotel_room as b on a.id=b.hotel_id left join
				tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
				on e.hotel_id=a.id where '.$get_id.' and c.available_status=1 group by a.title)';
		...
		{	
			...
			$sql_qry1='(select '.$select_values.' from tbl_hotel as a left join tbl_hotel_room as b on a.id=b.hotel_id left join
			tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
			on e.hotel_id=a.id where  ((a.destination like "%'.$keyword.'%")) '.$date_checkin.' '.$text_room.'  and c.available_status=1 )';
			...
			{ 
				...
				c.normal_allowed_kids='.$hotel_in_arr_new[$x]['normal_allowed_kids'].' and a.id= '.$hotel_in_arr_new[$x]['id'].' ';
					...
					$sql_qry1='(select '.$select_values.' from tbl_hotel as a left join tbl_hotel_room as b on a.id=b.hotel_id left join
					tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
					on e.hotel_id=a.id where ((a.destination like "%'.$keyword.'%")) '.$date_checkout.' '.$text_room.' '.$select_conditions .'and c.available_status=1)';
			 ...
			 if(!empty($get_room_id))
			{
				$sql_new_qry_date= '(select '.$select_values.' from tbl_hotel as a left join tbl_hotel_room as b on a.id=b.hotel_id left join
				tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
				on e.hotel_id=a.id  where '.$get_room_id.' and c.available_status=1 group by b.id)';
				$query_date_exe_rooms = $this->db->query($sql_new_qry_date);
				$hotel_date_arr_new=$query_date_exe_rooms->result_array(); 
	...
	function all_hotels_search_based($post, $star='',$price_range='', $loc_range='' , $fac_range='', $hotName_range='')
	{
		...
		tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
		on e.hotel_id=a.id where a.title like "%'.$keyword1.'%" '.$dat_query.' '.$text.' and c.available_status=1 group by a.title)';
		...
		"%'.$keyword1.'%" OR d.district like "%'.$keyword1.'%" OR d.state like "%'.$keyword1.'%")) '.$dat_query.' '.$text.' and c.available_status=1 group by a.title)';
		}
		else
		{
			$sql_qry1='';
		...
		"%'.$keyword2.'%" OR d.district like "%'.$keyword2.'%" OR d.state like "%'.$keyword2.'%")) '.$dat_query.' '.$text.' and c.available_status=1 group by a.title )';
		}
		else
		{
			$sql_qry2='';
		...
		"%'.$keyword3.'%" OR d.district like "%'.$keyword3.'%" OR d.state like "%'.$keyword3.'%")) '.$dat_query.' '.$text.' and c.available_status=1 group by a.title ) ';
		...
		"%'.$keyword4.'%" OR d.district like "%'.$keyword4.'%" OR d.state like "%'.$keyword4.'%")) '.$dat_query.' '.$text.' and c.available_status=1 group by a.title ) ';
		}
		else
		{
			$sql_qry4='';
		...
		tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
		on e.hotel_id=a.id where ('.$keyword.' or (a.address like "%'.$keyword1.'%" OR a.title like "%'.$keyword1.'%" OR a.place like 
		"%'.$keyword1.'%" OR d.district like "%'.$keyword1.'%" OR d.state like "%'.$keyword1.'%")) '.$dat_query.' '.$text_room.' and c.available_status=1 group by b.id)';
		}
		else
		{
			$sql_qry1='';
		...
		tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
		on e.hotel_id=a.id where ('.$keyword.' or (a.address like "%'.$keyword2.'%" OR a.title like "%'.$keyword2.'%" OR a.place like 
		"%'.$keyword2.'%" OR d.district like "%'.$keyword2.'%" OR d.state like "%'.$keyword2.'%")) '.$dat_query.' '.$text_room.' and c.available_status=1 group by b.id)';
		}
		else
		{
			$sql_qry2='';
		...
		tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
		on e.hotel_id=a.id where ('.$keyword.' or (a.address like "%'.$keyword3.'%" OR a.title like "%'.$keyword3.'%" OR a.place like 
		"%'.$keyword3.'%" OR d.district like "%'.$keyword3.'%" OR d.state like "%'.$keyword3.'%")) '.$dat_query.' '.$text_room.' and c.available_status=1 group by b.id ) ';
		...
		tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
		on e.hotel_id=a.id where ('.$keyword.' or (a.address like "%'.$keyword4.'%" OR a.title like "%'.$keyword4.'%" OR a.place like 
		"%'.$keyword4.'%" OR d.district like "%'.$keyword4.'%" OR d.state like "%'.$keyword4.'%")) '.$dat_query.' '.$text_room.' and c.available_status=1 group by b.id ) ';
		}
		else
		{
			$sql_qry4='';
		...
		$s_qry= '(select '.$select_values.' from tbl_hotel as a left join tbl_hotel_room as b on a.id=b.hotel_id left join
		tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
		on e.hotel_id=a.id where  '.$selescted_id_date.'  a.title like "%'.$keyword1.'%" '.$date_checkin.' '.$text.' and c.available_status=1 )';
		
		$sql_qry1='union (select '.$select_values.' from tbl_hotel as a left join tbl_hotel_room as b on a.id=b.hotel_id left join
		tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
		on e.hotel_id=a.id where '.$selescted_id_date.' ('.$keyword.' or (a.address like "%'.$keyword1.'%" OR a.title like "%'.$keyword1.'%" OR a.place like 
		"%'.$keyword1.'%" OR d.district like "%'.$keyword1.'%" OR d.state like "%'.$keyword1.'%")) '.$date_checkin.' '.$text.' and c.available_status=1 )';
		}
		else
		{
			$sql_qry1='';
		...
		"%'.$keyword2.'%" OR d.district like "%'.$keyword2.'%" OR d.state like "%'.$keyword2.'%")) '.$date_checkin.' '.$text.' and c.available_status=1)';
		}
		else
		{
			$sql_qry2='';
		...
		"%'.$keyword3.'%" OR d.district like "%'.$keyword3.'%" OR d.state like "%'.$keyword3.'%")) '.$date_checkin.' '.$text.' and c.available_status=1 ) ';
		}
		else
		{
			$sql_qry3='';
		...
		"%'.$keyword4.'%" OR d.district like "%'.$keyword4.'%" OR d.state like "%'.$keyword4.'%")) '.$date_checkin.' '.$text.' and c.available_status=1 ) ';
		}
		else
		{
			$sql_qry4='';
		...
		$keyword1 =  $query['text_val'][0];
				...
				$s_qry= '(select '.$select_values.' from tbl_hotel as a left join tbl_hotel_room as b on a.id=b.hotel_id left join
		tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
		on e.hotel_id=a.id where  '.$selescted_id_date.'  a.title like "%'.$keyword1.'%" '.$date_checkout.' '.$text.' '.$select_conditions.' and c.available_status=1 )';
	
		$sql_qry1='union (select '.$select_values.' from tbl_hotel as a left join tbl_hotel_room as b on a.id=b.hotel_id left join
		tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
		on e.hotel_id=a.id where '.$selescted_id_date.' ('.$keyword.' or (a.address like "%'.$keyword1.'%" OR a.title like "%'.$keyword1.'%" OR a.place like 
		"%'.$keyword1.'%" OR d.district like "%'.$keyword1.'%" OR d.state like "%'.$keyword1.'%")) '.$date_checkout.' '.$text.' '.$select_conditions.' and c.available_status=1 )';
		}
		else
		{
			$sql_qry1='';
		...
		tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
		on e.hotel_id=a.id where  '.$selescted_id_date.'  ('.$keyword.' or (a.address like "%'.$keyword2.'%" OR a.title like "%'.$keyword2.'%" OR a.place like 
		"%'.$keyword2.'%" OR d.district like "%'.$keyword2.'%" OR d.state like "%'.$keyword2.'%")) '.$date_checkout.' '.$text.' '.$select_conditions.' and c.available_status=1 )';
		}
		else
		{
			$sql_qry2='';
		...
		tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
		on e.hotel_id=a.id where '.$selescted_id_date.'  ('.$keyword.' or (a.address like "%'.$keyword3.'%" OR a.title like "%'.$keyword3.'%" OR a.place like 
		"%'.$keyword3.'%" OR d.district like "%'.$keyword3.'%" OR d.state like "%'.$keyword3.'%")) '.$date_checkout.' '.$text.'  '.$select_conditions.' and c.available_status=1 ) ';
		}
		else
		{
			$sql_qry3='';
		...
		tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
		on e.hotel_id=a.id where '.$selescted_id_date.'  ('.$keyword.' or (a.address like "%'.$keyword4.'%" OR a.title like "%'.$keyword4.'%" OR a.place like 
		"%'.$keyword4.'%" OR d.district like "%'.$keyword4.'%" OR d.state like "%'.$keyword4.'%")) '.$date_checkout.' '.$text.' '.$select_conditions.' and c.available_status=1 ) ';
		}
		else
		{
			$sql_qry4='';
		...
		{	
			...
			tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
			on e.hotel_id=a.id where ('.$keyword.' or (a.address like "%'.$keyword1.'%" OR a.title like "%'.$keyword1.'%" OR a.place like 
			"%'.$keyword1.'%" OR d.district like "%'.$keyword1.'%" OR d.state like "%'.$keyword1.'%")) '.$date_checkin.' '.$text_room.' and c.available_status=1 )';
			}
			else
			{
				$sql_qry1='';
			...
			{ 
				...
				if(!empty($query['text_val'][0]))
				{
					...
					$s_qry= '(select '.$select_values.' from tbl_hotel as a left join tbl_hotel_room as b on a.id=b.hotel_id left join
				tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
				on e.hotel_id=a.id where  '.$selescted_id_date.'  a.title like "%'.$keyword1.'%" '.$date_checkout.' '.$text_room.' '.$select_conditions.' and c.available_status=1 )';
			
					$sql_qry1='union (select '.$select_values.' from tbl_hotel as a left join tbl_hotel_room as b on a.id=b.hotel_id left join
					tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
					on e.hotel_id=a.id where ('.$keyword.' or (a.address like "%'.$keyword1.'%" OR a.title like "%'.$keyword1.'%" OR a.place like 
					"%'.$keyword1.'%" OR d.district like "%'.$keyword1.'%" OR d.state like "%'.$keyword1.'%")) '.$date_checkout.' '.$text_room.' '.$select_conditions .' and c.available_status=1)';
				}
				else
				{
					$sql_qry1='';
...
function Off_hotelTaxById($hotel)
{
	...
	{
		$query = $this->db->select('child_free_limit')
		->select('room_child_rate')->where('hotel_id', $id)
		->where('available_status', 1)->get('tbl_hotel_room_settings');
		return $query->result();
	...
	function star_count($value,$category,$res_category,$text_val,$destination)
	{
		...
		OR d.district like "%'.$keyword1.'%" OR d.state like "%'.$keyword1.'%")) '.$dat_query.' '.$text.' and c.available_status=1 group by a.title)';
		}
		else
		{
			 $sql_qry1='';
		...
		OR d.district like "%'.$keyword2.'%" OR d.state like "%'.$keyword2.'%")) '.$dat_query.' '.$text.' and c.available_status=1 group by a.title )';
		}
		else
		{
			$sql_qry2='';
		...
		OR d.district like "%'.$keyword3.'%" OR d.state like "%'.$keyword3.'%")) '.$dat_query.' '.$text.' and c.available_status=1 group by a.title ) ';
		...
		OR d.district like "%'.$keyword4.'%" OR d.state like "%'.$keyword4.'%")) '.$dat_query.' '.$text.' and c.available_status=1 group by a.title ) ';
		}
		else
		{
			$sql_qry4='';
	...
	{ 
		...
		(a.destination like "%'.$keyword.'%" )) '.$dat_query.' '.$text.' and c.available_status=1 group by a.title)';
		...
		if(!empty($this->session->userdata['datahere']['check_in']) && !empty($this->session->userdata['datahere']['check_out']))
				{
					...
					$i = 0;
							$date = $post['check_in'];
		while (strtotime($date) <= strtotime($post['check_out'])) {
					...
					$this->db->where('(("'. $date. '" BETWEEN a.check_in and a.check_out)) and (a.check_out != "'.$post['check_in'].'")');
		...
		!empty($book1) ? $booked_rooms1 = max($book1) : $booked_rooms1 = 0;
				...
				else
				{
				   ...
				   while (strtotime($date) <= strtotime($checkout)) {
					...
					$total[$i] = count($query_remaining->result());
		...
		$this->db->where('tbl_hotel_room_settings.available_status',1);
			...
			$select_values_new ='c.price,c.valid_from,c.valid_upto,c.id,c.normal_allowed_adults,c.normal_allowed_kids,c.room_id';
			 ...
			 $sql_new_qry_in= '(select '.$select_values_new.' from tbl_hotel as a left join tbl_hotel_room as b on a.id=b.hotel_id left join
				tbl_hotel_room_settings as c on b.id=c.room_id where a.id='.$hotel_id.'   '.$date_checkin.' and (c.allowed_persons="'.$person.'" or c.allowed_persons>"'.$person.'") and c.available_status=1 )'; 
			    $query_in_exe_rooms = $this->db->query($sql_new_qry_in);
				 $hotel_in_arr_new=$query_in_exe_rooms->result();
				 //echo count($hotel_in_arr_new);;
			...
			{  
				...
				tbl_hotel_room_settings as c on b.id=c.room_id where a.id='.$hotel_id.'   '.$date_checkout.'  '.$select_conditions.' and c.available_status=1 )';
				$query_out_exe_rooms = $this->db->query($sql_new_qry_out);
	...
	function destination_count($sub_category,$res_sub_category,$destination)
	{
		...
		$text ='and a.sub_category_id="'.$post['sub_cat_id'].'"  and a.status="1" ';
		 ...
		 $sql_qry1='(select '.$select_values.' from tbl_hotel as a left join tbl_hotel_room as b on a.id=b.hotel_id left join
		tbl_hotel_room_settings as c on b.id=c.room_id left join tbl_locations as d on d.id=a.state left join tbl_hotel_facilities as e 
		on e.hotel_id=a.id where ((a.destination like "%'.$keyword.'%" )) '.$text.' and c.available_status=1 group by a.title)';
