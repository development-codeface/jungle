...
class Hotelz extends CI_Controller {
	 ...
	 function _remap($name) {
	  ...
	  if(!empty($this->session->userdata['datahere']['text_val'])){
		$this->session->unset_userdata['datahere'];
	 }
	...
	public function index($parameter)
	{
		...
		if(!empty($refer))
		{
			$this->session->unset_userdata('refer_url2');
		 	redirect($refer);
	}
			 }
		...
		if($parameter=='hotels')
		{
			$res_cat = $this -> Hotelz_model -> get_id('resorts');
			$data['res_cat_id'] = $res_cat->id;
