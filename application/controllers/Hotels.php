...
class Hotels extends CI_Controller
{
	 ...
	 function result_sort($b_result, $hidden_search) {
	 
	 $hid = explode(',', $hidden_search);
	 	$g=0;
 foreach ($b_result as $h):		
$matches[$g] = preg_grep('~' . $hid[0] . '~i', $h);
$g++;
endforeach;

$h=0;
foreach(array_keys(array_filter($matches)) as $j):
$ids[$h] = $b_result[$j];
$h++;
endforeach;

if(!empty($ids)) {
$c_result = array_merge($ids,$b_result);
return array_unique($c_result, SORT_REGULAR);
} else {
return array_unique($b_result, SORT_REGULAR);
}
		//print_r(array_unique($c_result, SORT_REGULAR));
	 }
