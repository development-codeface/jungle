...
function coupon($cp,$in,$out) {

	$day = date('Y-m-d');
	
	$check1 = $this -> Checkout_model -> coupon($cp, NULL, NULL);
	if ($this->input->is_ajax_request()) {
	
	if(!empty($check1) && $check1 -> type == 0) {
	$check2 = $this -> Checkout_model -> coupon($cp, $in, $out);
	if(!empty($check2)) {
	echo $check2-> coupon_code;
	}
	}
	
	if(!empty($check1) && $check1 -> type == 1) {
	$check3 = $this -> Checkout_model -> coupon($cp, $day, NULL);
	if(!empty($check3)) {
	echo $check3 -> coupon_code;
	}
	}
	} else {
	...
	public function index()
	{
		...
		$datas['ck_data']=$this->input->get('ck_data');
		 ...
		 $datas['check_out'] = $this->input->get('check_out');
		 $datas['check_in'] = $this->input->get('check_in');
		 $datas['coupon'] = $this->input->get('coupon');
...
////  PayU Success end ////
		 ...
		 $data['coupon_code'] = $this->input->get('coupon');
...
/** check coupon **/
									$check_cp = $this -> coupon($data['coupon_code'],$data['check_in'],$data['check_out']);
									
									if(!empty($check_cp)) {
									$check_amt = $this -> Checkout_model -> coupon($data['coupon_code'], NULL, NULL);
									
									if(!empty($check_amt)) {
									$cp_amt = (round($total_amount+$room_tax_amounts +$commision_total) * $check_amt -> offer) / 100;
									}
									} 
									if(!empty($cp_amt)) {
		$data['coupon_amount'] = $cp_amt;}
