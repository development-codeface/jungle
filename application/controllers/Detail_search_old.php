	...
	public function ayurvedas()
	{
		...
		// $room_count; exit;
			...
			if($data['sub_cat_id']==6) {
				echo '<script>window.location="'.base_url().'ayurveda/search_list?hidden-search='.$search_text.'&package='.$package.'&category='.$data['sub_cat_id'].'&check_in='.$data['check_in'].'&check_out='.$data['check_out'].'"</script>';
			} else {
				echo '<script>window.location="'.base_url().'hotel-packages/search_list?hidden-search='.$search_text.'&category='.$data['sub_cat_id'].'&check_in='.$data['check_in'].'&check_out='.$data['check_out'].'"</script>';
			}}
