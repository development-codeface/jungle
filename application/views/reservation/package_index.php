...
<?php
	          	if(!isset($this->session->userdata['log_username']))
				{?>
<a id="log_reserve" class="sign_in" data-toggle="modal" data-target="#login_model" href="#">Sign In</a>
<?php } else 
{?>
<a href="<?php echo base_url(); ?>reservation/user?hotel=<?php echo $this->input->get('hotel'); ?>">My Account</a>
<?php } ?>
</div>-->
...
$room_detail	= $this->Reservation_model->ayurveda_rooms($package_id);
...
<input type="hidden" name="source" id="id" value="package" />
<div class="col-md-6 col-xs-12 col-sm-12 fs ts" >
							<label>Package</label><br>


							<select value="" class="col-lg-12 select-pack form_search" name="package" id="package_eng_id">
							<option value=""> --Choose Package---</option>
							<?php 
							foreach($package as $package_id)
							{  $package_name= $this->Reservation_model->package_name($package_id->id);?>
							
							<option value="<?php echo $package_id->id;?>" <?php if($package_id->id==$pack_id){?> selected="selected" <?php }?>><?php echo $package_name[0]->title;?></option>
																	
							<?php }?>								
							</select>
						
							</div>
<div class="col-md-3 col-xs-6 col-sm-6   fs top-packk width-check" id="sandbox-container"><label>Check In</label>
...
<div class="col-md-3 col-xs-6 col-sm-6   fs  top-packk width-check" id="sandbox-container4"><label>Check Out</label>
				                        	<input class="col-lg-8 form-control datepickr_ico" value="<?php if(!empty($check_out)){ echo $check_out; }?>" placeholder="Check-out" name="check_out" id="valid_to">
				                       </div>
...
<div class="search_f_hr3 col-md-5 pull-right">	
<div id="error_pacakge"style="display : none;">Please Select a Package</div>
 	...
 	<div class="container-fluid">
						  		...
						  		<div class="col-md-9 pull-right right-pad">
									...
									<div class="form-group">
								    	...
								    	<button type="submit" class="btn btn-primary pull-right hotel_search_engine" id="search_hotels" > SEARCH </button>
...
<div class="col-md-10 remv_p_div" id="">
				...
				{	
				//print_r($room);
					$img_room = $this->Reservation_model->imagesById($room->roomid);
					$fac_room = $this->Reservation_model->facilitiesById($room->roomid); $i=0; $j=0;
					$room_setting_detail	= $this->Reservation_model->ayur_settingsByRoom($room->roomid, $pack_details->ayurveda_id);	
	...
	<div class="img_block1_selected_room_list">
		...
		<span class="info label label-danger">Room Not Available For Search date</span>
			...
			{ if(!empty($room_setting_detail[0]->conditions)) {?>
				<p class="cal_tooltip cal_tooltipcusr btncusscan cancel_l" data-toggle="modal" data-target="#myModal_cancel<?php echo $room->roomid ?>"><i class="fa fa-ban" aria-hidden="true"></i> Cancellation Policy</p>	
				
			<?php } ?>
				<!-- Modal -->
			<div id="myModal_cancel<?php echo $room->roomid ?>" class="modal fade" role="dialog">
			...
			{  if(!empty($room_setting_detail[0]->services)) { ?>
				<p class="cal_tooltip cal_tooltipcusr btncusscan service_l" data-toggle="modal" data-target="#myModal<?php echo $room->roomid ?>"><i class="fa fa-location-arrow" aria-hidden="true"></i> Services Offered</p>
			<?php }?>
		<!-- Modal -->
			<div id="myModal<?php echo $room->roomid ?>" class="modal fade" role="dialog">
			  <div class="modal-dialog" style="margin-top: 132px;">
			...
			<div class="s_show_morecus roomDet" data-toggle="modal" data-target="#modalroomPack<?php echo $room->roomid ?>"><i class="fa fa-info-circle" aria-hidden="true"></i> Room Detail</div>
			
			  
			  <div id="modalroomPack<?php echo $room->roomid ?>" class="modal fade" role="dialog">
			  <div class="modal-dialog" style="margin-top: 132px;">
...
<table style="width:100%" class="sltd_rooms_details_lis">
		<thead>
		<tr>
				...
				<?php 	
										...
										<span class="info label label-danger">Room Not Available</span>
											...
											else
											{
								      			...
								      			$room_count=$this->Reservation_model->rooms_count($details->id,$room->roomid);
