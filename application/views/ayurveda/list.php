						...
						<form role="form" method="get" id="hotels_modify"  class="modufy-search_form_cus col-md-12" action="<?php echo base_url().$sub_cat;?>/search_list">
							...
							<div class="col-md-6">
							  <div class="form-group">
							    ...
							    <input type="text" name="hidden_search" onchange="destination_change();"  value="<?php echo $text_val; ?>" class="form-control" id="hidden_search" placeholder="Enter City name, Area name or Hotel name">
				...
				foreach ($b_result as $hotel_table) {
					...
					if(!empty($hotel_table['ayur_id']))
					{
						...
						$out =date_format(date_create_from_format('d-m-Y',$check_out ), 'Y-m-d');
							 ...
							 $room_price_check=$this->Ayurveda_model->room_price_lowest($hotel_table['ayur_id'],$in,$out);
						...
						else
						{
							...
							if(!empty($check_in) && !empty($check_out))
							{
								$offer_percentage= $this->Ayurveda_model->hotelofferById($hotel_table['hotel_id'],$check_in,$check_out);
							}
								$agency_rate=$this->Ayurveda_model->hotelAgencyById($hotel_table['hotel_id']);
								$agency_commission=$this->Ayurveda_model->hotelAgencyCommissionById($hotel_table['hotel_id']);
						...
						?>	
							...
							<div class="result_details_bottom col-md-12 select_room_cus_12">
								<div class="pull-right room_features">
									...
									<span data-toggle="tooltip" data-placement="top" title="Restaurant available"><i class="fa fa-cutlery"></i>  
									Restaurant
									</span>
