...
<?php   $this->load->view('layout/menu');  
			...
			<div class="row">
				...
				<div class="col-md-9 cl-b">
					...
					<div class="search_details" style="border-left: 3px solid #aaaaaa; ">
						...
						<form role="form" method="get"  id="hotels_modify"  class="modufy-search_form_cus col-md-12" action="<?php echo base_url();?>hotels/search_list">
  ...
  <input type="hidden" name="res_category" id="res_sub_catid" value="<?php echo $res_sub_cat_id;?>"/>
							    <input type="text" name="hidden-search" value="<?php echo $text_val; ?>" class="form-control" id="hidden_search" placeholder="Enter City name, Area name or Hotel name">
						  ...
						  <input type="hidden" name="category" id="sub_catid" value="<?php echo $sub_cat_id;?>"/>
							...
							<div class="search_alert" style="display : none;">Please Enter search content here</div>
							
						  	
						  	
						  	<div class="col-md-12 modufy-search-cus_block2 pad_out_det">
							  	<div class="col-md-3 modufy_search_block_cus res7 room_top_det">
							  		<label for="pwd" class="rooms_cus_title rmNamedet">Rooms</label>
...
<!-- ajax -->
	...
	<?php if(!empty($this->session->userdata['datahere']['check_in']) && !empty($this->session->userdata['datahere']['check_out'])) { ?>
					...
					$persons =array();
					for($a=0;$a<$mem_type;$a++)
					{ 
						//array_push($persons,($adults[$a]+$childrens[$a]));
						array_push($persons,$adults[$a]);
					}
					$person_low = (min($persons));
					
					if(!empty($hotel[1][$i]['settings_id']))
					{
						...
						$out =date_format(date_create_from_format('d-m-Y',$check_out ), 'Y-m-d');
							 ...
							 $room_price_check=$this->Hotels_model->room_price_lowest($hotel[1][$i]['id'],$in,$out,$person_low);
						...
						if($tot_room >= 1)
						{
							...
							if(!empty($check_in) && !empty($check_out))
							{ 
								$offer_percentage= $this->Hotels_model->hotelofferById($hotel[1][$i]['id'],$check_in,$check_out);
								
							}
								$agency_rate=$this->Hotels_model->hotelAgencyById($hotel[1][$i]['id']);
								$agency_commission=$this->Hotels_model->hotelAgencyCommissionById($hotel[1][$i]['id']);
...
///////////////////Offer Price Calculation///////////////////////////////////////////
	...
	///////////////Calcualting the Grand Total////////////////////////////////////////	
								  ...
								  //$room_price=$agency_price+$child_rate+$adult_rate;
								    $rm_price=$agency_price+$child_rate+$adult_rate;
						}
					}
					if(!empty($rm_price)) {
...
<div class="search_result  search_result_cus_block col-md-12">
					...
					{
							?>
							<h1><a href="<?php echo base_url();?>hotels/detail/<?php echo $enc_id;?>"><?php echo $hotel_table['title']; ?></a></h1>								
				...
				?>
						...
						<div class="col-md-3 book_now select_room_cus">
							...
							<?php if(!empty($this->session->userdata['datahere']['check_in']) && !empty($this->session->userdata['datahere']['check_out']))
								...
								<h2><?php if($room_price!=''){?><span class="fa fa-<?php if(!$this->session->userdata('currency')) echo "inr"; else echo  $this->session->userdata('currency'); ?>" data-rating="1"></span> 
								<?php } 
						...
						if(!empty($this->session->userdata['roomprice'.$hotel_table['id']]['total']))
						{
							...
							<a href="<?php echo base_url();?>hotels/detail/<?php echo $enc_id;?>" class="btn btn-primary select-but"> SELECT ROOM </a> 
